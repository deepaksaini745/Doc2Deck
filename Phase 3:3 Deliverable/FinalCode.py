...
from prompt_templates import ENRICH_PRESENTATION_PROMPT, SLIDE_REFINEMENT_PROMPT, EXTRACT_TOPICS_MARKERS_TEMPLATE
...
def make_api_call_gpt(api_key, content, retries=3):
    import openai
    ...
    for attempt in range(retries):
        try:
            ...
            if attempt < retries - 1:
                ...
                print("[Rate Limit] Giving up after retries.")
                return None
...
def extract_document_data(doc_path):
    ...
    for idx, image_doc in enumerate(images):
        ...
        # Skip blank images by checking if they are mostly white
        try:
            with Image.open(image_path) as img:
                img_rgb = img.convert("RGB")
                colors = img_rgb.getcolors(maxcolors=1000000)
                if colors:
                    dominant_ratio = max([count for count, _ in colors]) / (img.size[0] * img.size[1])
                    if dominant_ratio > 0.98:
                        print(f"[DEBUG] Skipping solid-color image {image_path} (dominant_ratio={dominant_ratio:.2f})")
                        solid_color_skips += 1
                        continue
                ...
                if white_ratio > 0.98:
                    print(f"[DEBUG] Skipping blank white image {image_path} (white_ratio={white_ratio:.2f})")
                    white_image_skips += 1
                    continue
                    # print(f"[DEBUG] Skipping blank image {image_path} (white_ratio={white_ratio:.2f})")
                    # continue
        except Exception as e:
    ...
    print(f"[DEBUG] Number of solid-color images skipped: {solid_color_skips}")
    print(f"[DEBUG] Number of white-space images skipped: {white_image_skips}")
    ...
    print(f"[DEBUG] Number of tables extracted: {len(tables)}")
        ...
        # If no tables extracted, try asking GPT to find markdown tables
    if not tables:
        print("[INFO] No structured tables found, sending text to GPT to extract any Markdown tables.")
        markdown_prompt = textwrap.dedent(f"""
            The following document contains tabular data. Your task is to extract each full table completely and return it in valid Markdown format.

            - Ensure all rows that belong to the same table appear together in a single Markdown block.
            - Do not split rows of the same table across multiple Markdown tables.
            - If the same header row appears again, consider it the start of a new table.
            - Return only Markdown tables. No explanations or labels.

            Document:
            {text}
        """)
        markdown_response = make_api_call_gpt(os.getenv("OPENAI_API_KEY"), markdown_prompt)
        if markdown_response:
            guessed_tables = []
            for table_md in markdown_response.strip().split("\n\n"):
                if '|' in table_md:
                    rows = [r.strip().strip('|').split('|') for r in table_md.strip().splitlines() if '|' in r]
                    guessed_tables.append([[cell.strip() for cell in row] for row in rows])
            tables.extend(guessed_tables)

            # Remove table content from text to avoid duplicate bullet slides
            for table in guessed_tables:
                for row in table:
                    for cell in row:
                        if cell.strip() and cell.strip() in text:
...
def refine_gpt_slide_output(slides):
    refine_prompt = SLIDE_REFINEMENT_PROMPT.replace("{original_slide_json}", json.dumps(slides, indent=2))
    refined_response = make_api_call_gpt(os.getenv("OPENAI_API_KEY"), refine_prompt)
    if refined_response:
        try:
            print("[INFO] Using refined response instead of original slides.")
            return json.loads(refined_response)
        except json.JSONDecodeError:
            print("[WARNING] Refined response is not valid JSON. Using original slides.")
            return slides
    else:
        print("[WARNING] No refined slide response from GPT. Using original slides.")
        return slides

def enrich_with_gpt(document_data, topics):
...
def create_ppt_from_gpt(gpt_slides, document_data, ppt_path="generated_deck.pptx", template_path=None):
    seen_tables = []
    ...
    for slide_data in gpt_slides:
        # Skip slides that will be empty (table already used, no text, no image)
        is_duplicate_table = False
        if slide_data.get("table"):
            for prev_table in seen_tables:
                if slide_data["table"] == prev_table:
                    is_duplicate_table = True
                    break

        if not slide_data.get("text") and not slide_data.get("image") and (not slide_data.get("table") or is_duplicate_table):
            continue
        ...
        if slide_data.get("title") and slide.shapes.title:
            ...
            if title_length <= 50:
                ...
                font_size = Pt(20)
            title_frame.paragraphs[0].font.size = font_size
        ...
        if len(slide.placeholders) > 1:
            ...
            if not slide_data.get("image") or not os.path.exists(slide_data["image"]):
                text_box.left = Inches(1.0)
                text_box.top = Inches(2.0)
                text_box.width = slide_width - Inches(1.5)
                text_box.height = slide_height - Inches(2.5)
            else:
                text_box.left = text_box_left
                text_box.top = text_box_top
                text_box.width = text_width  - Inches(0.1)
                text_box.height = text_box_height - Inches(0.5)
        ...
        if slide_data.get("table"):
            for prev_table in seen_tables:
                if slide_data["table"] == prev_table:
                    is_duplicate_table = True
                    break
            if not is_duplicate_table:
                seen_tables.append(slide_data["table"])

        if slide_data.get("table") and not slide_data.get("text") and not is_duplicate_table:
            if len(slide.placeholders) > 1:
                text_box = slide.placeholders[1]
                sp = text_box._element
                sp.getparent().remove(sp)  # Remove placeholder if only table present
...
def generate_image_captions(image_paths):
    ...
    for idx, image_path in enumerate(image_paths):
        print(f"[INFO] Captioning image {idx+1}/{len(image_paths)}: {image_path}")
        ...
        try:
            ...
            # print(f"[INFO] Caption for {image_path}: {caption}")
            captions[image_path] = caption
            print(f"[DEBUG] Caption: {caption}")

        except Exception as e:
            print(f"[WARNING] Failed to caption image {image_path}: {e}")

        wait_time = random.uniform(1, 2)
        print(f"[INFO] Sleeping {wait_time:.1f}s before next image...")
        time.sleep(wait_time)

    return captions

def map_images_to_slides(slides, image_captions):
    mapped_images = {}
    used_images = set()
    ...
    for idx, slide in enumerate(slides):
        ...
        for image_path, caption in image_captions.items():
            if image_path in used_images:
                continue
            score = fuzz.partial_ratio(slide_text.lower(), caption.lower())
            if score > best_score:
                best_score = score
                best_match = image_path

        if best_match and best_score > 50:
            mapped_images[idx] = best_match
...
def main():
    ...
    # New Step: Generate captions for extracted images
    image_captions = generate_image_captions(document_data['images'])
    with open(os.path.join(intermediate_dir, "image_captions.json"), "w", encoding="utf-8") as f:
        json.dump(image_captions, f, ensure_ascii=False, indent=4)

    # CHUNKING TEXT
    text_chunks = split_text_into_chunks(document_data['text'], max_words=2500)

    all_gpt_responses = []
    for idx, chunk_text in enumerate(text_chunks):
        print(f"[INFO] Processing chunk {idx+1}/{len(text_chunks)}")
        ...
        try:
            topics = extract_topics_from_gpt(chunk_text)
            gpt_response = enrich_with_gpt(temp_doc_data, topics)

            if gpt_response:
                refined_response = refine_gpt_slide_output(gpt_response)
                all_gpt_responses.extend(refined_response)

        except ValueError as e:
            print(f"[ERROR] Skipping chunk {idx+1} due to topic extraction failure: {e}")

        wait_time = random.uniform(4, 7)
        print(f"[INFO] Sleeping {wait_time:.1f}s before next chunk...")
        time.sleep(wait_time)

    with open(os.path.join(intermediate_dir, "gpt_structured_response.json"), "w", encoding="utf-8") as json_file:
        json.dump(all_gpt_responses, json_file, ensure_ascii=False, indent=4)

    # Map images to slides semantically
    slide_to_image_mapping = map_images_to_slides(all_gpt_responses, image_captions)
    print(f"[INFO] Slide to Image Mapping: {slide_to_image_mapping}")

    create_ppt_from_gpt(all_gpt_responses, document_data, output_ppt_path, template_path)
