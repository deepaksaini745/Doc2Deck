...
def extract_topics_from_gpt(document_text):
    from prompt_templates import EXTRACT_TOPICS_MARKERS_TEMPLATE
...
def enrich_with_gpt(document_data):
    ...
    text_chunks = split_text_into_chunks(document_data['text'])
    structured_response = []
    for idx, chunk in enumerate(text_chunks):
        ...
        print(f"[INFO] Sending chunk {idx+1}/{len(text_chunks)} to GPT...")
        response = make_api_call_gpt(os.getenv("OPENAI_API_KEY"), partial_prompt)
        if response:
            try:
                structured_response.extend(json.loads(response))
            except json.JSONDecodeError:
                print(f"[WARNING] Chunk {idx+1} returned invalid JSON and was skipped.")
        else:
            print(f"[WARNING] No response for chunk {idx+1}, skipping.")

    if not structured_response:
        raise ValueError("GPT API call failed. No response received.")
