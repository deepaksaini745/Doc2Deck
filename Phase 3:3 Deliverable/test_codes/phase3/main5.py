...
def extract_topics_from_claude(document_text):
    ...
    response = make_api_call(ANTHROPIC_API_KEY, prompt)
...
def main():
    doc_path = os.path.join(base_dir, "input", "doc.pdf")
    ...
    with open(os.path.join(intermediate_dir, "claude_structured_response.json"), "w", encoding="utf-8") as json_file:
