...
def segment_content(text, topics, tables):
    segments = []
    table_index = 0
    for i, item in enumerate(topics):
        ...
        if start < end:
            ...
            if table:
                table_index += 1
            segments.append({"topic": item['topic'], "content": segment, "table": table})
...
def generate_slide_contents(segments):
    ...
    for segment in segments:
        ...
        if slide_text:
            slides.append({"topic": segment['topic'], "text": slide_text, "table": segment['table']})
        else:
    ...
    for slide_data in slides:
        ...
        for line in lines:
            ...
            if not line.startswith("•") and not line.startswith("-"):
                ...
                # Add table if available for this slide_data
                if slide_data.get("table"):
                    add_table_to_slide(current_slide, slide_data["table"])
                ...
                # Add bullet point to the current slide
                text_box = current_slide.placeholders[1]
                text_frame = text_box.text_frame
...
def main():
    ...
    segments = segment_content(document_data["text"], topics, document_data["tables"])
