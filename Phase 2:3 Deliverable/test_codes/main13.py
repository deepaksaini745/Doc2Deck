...
def extract_document_data(doc_path):
    ...
    # Ensure images directory is clean
    ...
    # Context-based renaming for cleanliness
    for idx, image_path in enumerate(image_paths):
        page_index = idx  # fallback, not reliable
        context_text = "general_context"  # fallback

        if idx < len(result.pages):
            context_text = result.pages[idx].text.lower().strip().replace(" ", "_").replace("/", "_")[:50]
        ...
        image_paths[idx] = new_path  # Update path to renamed file
...
def create_ppt_from_claude(claude_slides, document_data, ppt_path="generated_deck.pptx", template_path=None):
    ...
    for slide_data in claude_slides:
        ...
        # Contextual image assignment based on slide title
        assigned_image = None
        ...
        if assigned_image:
            ...
            print(f"Contextually assigned image: {assigned_image}")
        else:
