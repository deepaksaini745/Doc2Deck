...
def extract_document_data(doc_path):
    # Clear the images directory before each run
    ...
    for idx, image_path in enumerate(image_paths):
        ...
        if idx < len(result.pages):
            context_text = result.pages[idx].text.lower().strip().replace(" ", "_").replace("/", "_")[:40]
...
def create_ppt_from_claude(claude_slides, document_data, ppt_path="generated_deck.pptx", template_path=None):
    ...
    for slide_data in claude_slides:
        ...
        if slide_data.get("text"):
            has_image = slide_data.get("image") and os.path.exists(slide_data["image"])
            if len(slide.placeholders) > 1:
                ...
                if has_image:
                    text_box.width = Inches(5.9)
                    text_box.left = Inches(0.9)
                    text_box.top = Inches(2.0)
                    text_box.height = Inches(4.0)
